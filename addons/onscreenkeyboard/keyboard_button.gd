...
func _ready():
	touchscreenb=TouchScreenButton.new()
	var shape=RectangleShape2D.new()
	
	shape.extents=rect_size/2
	touchscreenb.shape=shape
	add_child(touchscreenb)
	
	touchscreenb.connect("pressed",self,"button_up")
	touchscreenb.connect("released",self,"button_down")
